...
local function AddToTrunk(plate, slot, otherslot, itemName, amount, info)
	...
	TriggerClientEvent('inventory:client:ItemBox',source,ItemData,'remove', amount) 
...
RegisterNetEvent('inventory:server:combineItem', function(item, fromItem, toItem)
	...
	TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[fromItem.name], 'remove', 1)
	TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[toItem.name], 'remove', 1)

	ply.Functions.AddItem(item, 1, nil, {["quality"] = 100})
...
--GIVE
RegisterNetEvent("inventory:server:GiveItem", function(target, inventory, item, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	...
	if Config.playerammo then 
		MySQL.update("SELECT * FROM `playerammo` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(ammo)
			if ammo[1] ~= nil then
	...
	if name ~= nil and id ~= nil then
		...
		elseif name == "otherplayer" then 
			TriggerClientEvent("inventory:client:closeClothing", source)
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerClientEvent('inventory:client:ItemBox', src, itemInfo, "add", toAmount)
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "trunk" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				TriggerClientEvent('inventory:client:ItemBox',source,QBCore.Shared.Items[itemInfo.name],'add', fromAmount) 
		...
		if QBCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items[itemData.name], "add", 1)
				else
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items[itemData.name], "add", fromAmount)
	...
	elseif fromInventory == "attachment_crafting" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items[fromItemData.name], "add", fromAmount)
	...
	if Player ~= nil then
		if amount > 0 then
			if itemData ~= nil then
				...
				elseif itemData['name'] == 'murder-meal' then
					info.mealid = math.random(11111,99999)
				elseif itemData["name"] == "labkey" then
					info.lab = exports["qb-methlab"]:GenerateRandomLab()
				elseif itemData["name"] == "fishicebox" then
					info.boxid = math.random(111,999)
					info.boxOwner = Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname
				...
				if Player.Functions.AddItem(itemData["name"], amount, false, info) then
					...
					TriggerClientEvent('inventory:client:ItemBox', source, itemData, "add", amount)

					TriggerClientEvent('QBCore:Notify', source, "You have given " ..GetPlayerName(tonumber(args[1])).." "..amount.. " " ..itemData["name"] , "success")

				else
					TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Can't give item!")
				end
			else
